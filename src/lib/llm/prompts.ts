...
- **1つの発話から複数項目を抽出してよい**（むしろ積極的にそうする）
  ...
  - 例: "自転車を週1で捨てたい" → items[0]={label:"自転車",frequency:"週1日"} ※ frequency は固定 enum ではなく自由表現 OK（"週3日" "土日のみ" 等そのまま入れる）
...
- **label は具体的な品目名のみ**。物の正体が特定できる名詞を入れる
  ...
  - OK 例（事業ゴミ分類名）: "可燃ゴミ" "不燃ゴミ" "ビン・缶・ペットボトル" "段ボール・古紙" "古紙" "汚泥" "廃プラ" "食品残渣" "廃食用油" "木くず" "金属くず" — 事業者フローでは分類名が品目になるので **総称ワード扱いせず label として OK**
  - NG 例: "粗大ゴミ" "不用品" "ゴミ" "廃棄物" "回収品" "もの" "あれ" "それ"（あいまいな総称・指示語のみ）
- ユーザーが **あいまいな総称ワード単独** （例: 「粗大ゴミ捨てたい」「不用品の回収お願い」「ゴミを処分したい」）で発話した場合、**items は patch に含めない**（state machine が後で「捨てたいものを教えてください」と具体的に聞き直すため）
  - 一方で「可燃ゴミ」「段ボール」のように分類が明確な単語は label として登録する
...
- frequency: 固定 enum ではなく自由表現の文字列。推奨候補例 "毎日" / "週3日" / "毎週月曜" / "隔週水曜" / "月2回" / "毎月第3月曜" / "土日のみ" / "月末締め" 等。ユーザーの表現をそのまま入れて構わない
